...
if (!app.Environment.IsDevelopment())
{
 ...
 // HSTS: The default HSTS value is30 days
 app.UseHsts();
}
...
// Esto permite ejecutar las migraciones de EF Core al iniciar la aplicación
using (var scopeForMigrations = app.Services.CreateScope())
{
 var servicesForMigrations = scopeForMigrations.ServiceProvider;
 try
 {
 var db = servicesForMigrations.GetRequiredService<ApplicationDbContext>();
 // Aplicar migraciones pendientes
 db.Database.Migrate();
 }
 catch (Exception ex)
 {
 var logger = servicesForMigrations.GetRequiredService<ILogger<Program>>();
